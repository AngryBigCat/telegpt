...
async def start_private_chat(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    CONVERSION_ID = "214a63e7-6025-4db8-9ea5-f92d811a6525"
    
    try:
        # 读取parent_message_id
        parent_message_id = '';
        with open('./parent_message_id', 'r') as file:
            parent_message_id = file.read()
        if len(parent_message_id) == 0:
        ...
        # 发送http请求
        chaturl = "http://chat.zuiax.com/api/conversation/talk"
        data = {
            "prompt": str(prompt),
            "stream": False,
        ...
        response = requests.post(chaturl, headers=headers, json=data, timeout=60)
        ...
        # 保存message_id到parent_message_id文件
        with open("./parent_message_id", 'w') as file:
            file.write(response_data['message']['id'])
